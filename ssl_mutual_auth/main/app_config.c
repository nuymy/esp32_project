...
provision_typedef_t provision_type = PROVISION_ACCESSPOINT;
...
void app_config (void) // just work with wifi 2.4Ghz because wifi 5GHz not working with esp32
{
    ...
    bool provisioned = is_provisioned();//check wifi config
    if (!provisioned)
    {
